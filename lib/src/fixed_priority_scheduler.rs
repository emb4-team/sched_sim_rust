...
use crate::{graph_extension::NodeData, log::*, processor::ProcessorBase, scheduler_base::*};
...
mod tests {
    ...
    fn test_fixed_priority_scheduler_log_normal() {
        ...
        // start_time
        assert_eq!(
            yaml_doc["node_logs"][0]["event_time"].as_str().unwrap(),
            "0"
        );
        // finish_time
        assert_eq!(
            yaml_doc["node_logs"][1]["event_time"].as_str().unwrap(),
            "52"
        );
