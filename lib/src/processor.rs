...
pub fn get_minimum_multiplier_from_dag_set(dag_set: &Vec<Graph<NodeData, f32>>) -> i32 {
    fn round_fraction(num: f32, decimal_places: u32) -> f32 {
        let multiplier = 10_f32.powi(decimal_places as i32);
        (num * multiplier).round() / multiplier
    }
    let mut minimum_multiplier = 1;
    ...
    for dag in dag_set {
        for node_index in dag.node_indices() {
            ...
            if exec_time.fract() != 0.0 {
                ...
                let new_minimum_multiplier = minimum_multiplier;
                if new_minimum_multiplier > minimum_multiplier {
                    minimum_multiplier = new_minimum_multiplier;
                }
            }
        }
    }
    minimum_multiplier as i32
}
...
mod tests {
    ...
    #[test]
    fn test_get_minimum_minimum_multiplier_from_dag_set() {
        let mut dag0 = Graph::<NodeData, f32>::new();
        dag0.add_node(create_node(0, "execution_time", 30.0));

        let mut dag_set = vec![dag0];
        assert_eq!(get_minimum_multiplier_from_dag_set(&dag_set), 1);
        ...
        assert_eq!(get_minimum_multiplier_from_dag_set(&dag_set), 10);

        let mut dag3 = Graph::<NodeData, f32>::new();
        dag3.add_node(create_node(3, "execution_time", 0.03));
        dag_set.push(dag3);

        assert_eq!(get_minimum_multiplier_from_dag_set(&dag_set), 100);
    }
}
